...
function DisplayValue({ fieldDef, value }: { fieldDef: FieldDefinition; value: FieldValue }) {
  ...
  return <span className="text-xs font-medium text-slate-800 text-right whitespace-normal break-words">{String(value)}</span>;
}
            ...
            <>
              <div className="flex items-center justify-between py-2 border-b border-slate-100 gap-2">
                ...
                <div className="flex items-center gap-1 flex-1 justify-end min-w-0">
                  <span className="text-xs font-medium text-slate-800 text-right block whitespace-normal break-words">{lead.user.name}</span>
                </div>
              </div>
              <div className="flex items-center justify-between py-2 border-b border-slate-100 gap-2">
                ...
                {editingValue ? (
                  ...
                  <div className="flex items-center gap-1 flex-1 justify-end min-w-0">
                    <button
                      ...
                      className="text-xs font-medium text-slate-800 hover:text-af-mid transition-colors text-right whitespace-normal break-words"
                    >
                ...
                <div className="flex items-center gap-1 flex-1 justify-end min-w-0">
                  {editingKey === key ? (
                    ...
                    <button onClick={() => setEditingKey(key)} className="flex-1 text-right hover:text-af-mid transition-colors min-w-0">
                      {customValues[key]
                        ? <span className="text-xs font-medium text-slate-800 text-right block whitespace-normal break-words">
                            {(key === 'renda_1' || key === 'renda_2')
