    ...
    try {
      const visionRes = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1024,
          system: 'Você extrai dados de documentos de identificação e comprovantes brasileiros (CNH, RG, carteira de trabalho, contracheque/holerite etc). Responda SOMENTE com um JSON válido, sem markdown e sem texto antes ou depois, no formato exato: {"nomeCompleto": string|null, "cpf": string|null, "dataNascimento": string|null, "renda": string|null, "numeroDocumento": string|null}. CPF no formato 000.000.000-00; data de nascimento no formato DD/MM/AAAA; renda como número em texto (ex: "5000.00") e SÓ se o documento for claramente um comprovante de renda/holerite — senão null; numeroDocumento é o número de registro da CNH ou do RG (o número do documento em si, não o CPF), exatamente como escrito, SÓ se o documento for uma CNH ou RG — senão null. Se não conseguir ler algum campo com certeza no documento, use null nesse campo. NUNCA invente valores.',
          messages: [{
            role: 'user',
